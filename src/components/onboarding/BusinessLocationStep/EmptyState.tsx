...
const EmptyState = ({ onAddLocation }: EmptyStateProps) => {
  ...
  return (
    <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-lg bg-slate-50">
      ...
      <h3 className="text-lg font-medium text-slate-700 mb-2">{t('onboarding.businessLocation.emptyState.title')}</h3>
      <p className="text-sm text-slate-500 mb-6">{t('onboarding.businessLocation.emptyState.description')}</p>
      <Button 
      ...
      >
        ...
        {t('onboarding.businessLocation.addLocation')}
      </Button>
    </div>
  );
};
